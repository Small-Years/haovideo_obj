...
# typeName重复的字段判断※
# typeHeadUrl 其他字段
def remove_duplicate(dict_list,typeName,typeHeadUrl):
    seen = set()
    new_dict_list = []
    for dict in dict_list:
        t_dict = {typeName: dict[typeName]}
        t_tup = tuple(t_dict.items())
        if t_tup not in seen:
            seen.add(t_tup)
            new_dict_list.append({typeName: dict[typeName], typeHeadUrl: dict[typeHeadUrl]})
